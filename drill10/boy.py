...
class IdleState:
    ...
    @staticmethod
    def do(boy):
        boy.frame = (boy.frame + FRAMES_PER_ACTION * ACTION_PER_TIME * game_framework.frame_time) % 15
        boy.timer -= 1
        if boy.timer == 0:
    ...
    def draw(boy):
        if boy.dir == 1:
            boy.image.clip_draw(int(boy.frame) * 153, 168 * boy.frame, 100, 100, boy.x, boy.y)
        else:
            boy.image.clip_draw(int(boy.frame) * 153, 168 * boy.frame, 100, 100, boy.x, boy.y)
